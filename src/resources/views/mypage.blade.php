                                        ...
                                        <form action="{{ route('reservations.update', $reservation->id) }}"
                                            ...
                                            <div>
                                                ...
                                                <input type="time" name="time" id="time"
                                                    value="{{ \Carbon\Carbon::parse($reservation->start_at)->format('H:i') }}">
                                            </div>
                                            <div>
                                                <label for="num_of_users">人数:</label>
                                                <input type="number" name="num_of_users" id="num_of_users"
                                                    value="{{ $reservation->num_of_users }}" min="1">
                                            </div>

                                            <button type="submit" class="edit-button">変更を保存</button>
                                        </form>
                                    @else
                                        <!-- 変更するボタン -->
                                        <form action="{{ route('mypage') }}" method="GET" style="display:inline;">
                                            <input type="hidden" name="edit_reservation"
                                        ...
                                        <form action="{{ route('reservations.destroy', $reservation->id) }}"
                                            ...
                                            <button type="submit" class="delete-button"
                                                onclick="return confirm('この予約を取り消しますか？')">キャンセル</button>
                                        </form>
                                    @endif
                                </div>
                            </div>
